...
| IP                                                          | `ip.src`                                                                                                                  | [Incompatible with **IP with NAT support**](#incompatible-characteristics) and [IPv6 address handling](#ipv6-address-handling)                                                                                                                           |
...
| **Custom** (enter expression)                               | Enter a custom expression. You can use a function such as `substring()` or `cidr6()`, or enter a more complex expression. | [Functions](/ruleset-engine/rules-language/functions/)                                                                                                                                                                                                   |
...
<Render file="rate-limiting-ipv6-handling" product="waf" />

To perform rate limiting based on all IPv6 addresses with the same prefix, you can use the [`cidr6()`](/ruleset-engine/rules-language/functions/#cidr6) function to define a [custom characteristic](#with-the-same-characteristics) based on an expression — for example, `cidr6(ip.src, 64)`.
